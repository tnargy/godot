...
func spawn_enemies(wave_data):
	for i in wave_data:
		var new_enemey = load("res://Scenes/Enemies/" + i[0] + ".tscn").instantiate()
		new_enemey.connect("base_damage", Callable(self, 'on_base_damage'))
		map_node.get_node("Path3D").add_child(new_enemey, true)
